...
![Precision@K 对比图](eval_precision.png)

*图 2: Precision@K 对比图，展示不同 K 值下的检索精确率*

![Recall@K 对比图](eval_recall.png)
...
| 指标 | 值 | 说明 |
|------|-----|------|
| 平衡性 | 0.9485 | 哈希码中 +1/-1 的平衡程度 |
| 唯一性 | 0.7695 | 不同哈希码的区分度 |
| 平均汉明距离 | 0.4983 | 理想值接近 bit/2 |
| 稀疏性 | 0.0000 | 哈希码的非零比例 |

![哈希码质量雷达图](eval_hash_quality.png)
...
ASPE (Asymmetric Scalar-product-preserving Encryption) 是一种非对称标量积保持加密方案，用于保护跨模态哈希检索中的数据隐私。其核心特性包括：
...
|----------|----------|----------|------|
| i→t | 0.720043 | 0.720320 | 2.78e-04 |
| t→i | 0.756550 | 0.756537 | 1.26e-05 |

### 4.5 排序一致性验证

- **验证结果**: ✅ 通过

![ASPE 明文 vs 密文对比](aspe_comparison.png)
...
**实现**: 使用汉明距离排序，计算每个查询的 AP 后取平均。汉明距离通过内积计算：
```
hamm(B1, B2) = 0.5 × (bit - B1·B2)
```
...
**实现**: 基于汉明距离排序后的前 K 个结果，与真实标签比对计算。
...
- 计算: `1 - |mean(B)|`
- 意义: 平衡的哈希码能最大化信息容量，每个 bit 提供最大的区分度
...
- 定义: 任意两个哈希码之间的平均汉明距离
- 理想值: `bit / 2`（最大区分度）
- 计算: `Σ hamm(Bi, Bj) / (n×(n-1)/2)`
- 意义: 距离适中时，相似性度量最有效

**稀疏性 (Sparsity)**:
- 定义: 哈希码中 0 的比例（对于二进制哈希码应为 0）
- 意义: 验证哈希码确实为 {-1, +1} 二进制

### 5.4 ASPE 安全性验证

**加密原理**:
- 数据库点加密: `Enc(p) = M^T × (p, -0.5||p||²)`
- 查询点加密: `Enc(q) = M^(-1) × r×(q, 1)`
- 内积保持: `Enc(p)·Enc(q) = r × (p·q - 0.5×bit)`
...
本次评估使用 64 bits 哈希码，在 Flickr-25K 数据集上达到 **0.7383** 的平均 mAP。
