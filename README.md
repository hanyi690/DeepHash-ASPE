...
**问题背景**：
- `calc_loss` 返回未归一化的总损失，数值高达 10^8 ~ 10^11
- 原因：总损失对 `num_train^2` 个样本对求和
- 不影响训练稳定性，但难以直观监控训练趋势
...
| 指标 | 修改前 | 修改后 |
|------|--------|--------|
| Loss 数值 | 10^8 ~ 10^11 | ~0.7 (与 batch loss 一致) |
| 可读性 | 差 | 好 |
| 训练稳定性 | 不变 | 不变 |

**修改文件**：
- `training/train_dcmh.py`：第 360-374 行
